 ...
 * @method     ChildSubscriptionQuery orderBySubscriptionType($order = Criteria::ASC) Order by the subscription_type column
 *
 ...
 * @method     ChildSubscriptionQuery groupBySubscriptionType() Group by the subscription_type column
 *
 ...
 * @method     ChildSubscription findOneBySubscriptionType(string $subscription_type) Return the first ChildSubscription filtered by the subscription_type column *
 ...
 * @method     ChildSubscription requireOneBySubscriptionType(string $subscription_type) Return the first ChildSubscription filtered by the subscription_type column and throws \Propel\Runtime\Exception\EntityNotFoundException when not found
 *
 ...
 * @method     ChildSubscription[]|ObjectCollection findBySubscriptionType(string $subscription_type) Return ChildSubscription objects filtered by the subscription_type column
...
abstract class SubscriptionQuery extends ModelCriteria
{
     ...
     * </code>
     *
     * @param array[$user_id, $subscription_id, $subscription_type] $key Primary key to use for the query
    ...
    public function findPk($key, ConnectionInterface $con = null)
    {
        ...
        if ((null !== ($obj = SubscriptionTableMap::getInstanceFromPool(serialize([(null === $key[0] || is_scalar($key[0]) || is_callable([$key[0], '__toString']) ? (string) $key[0] : $key[0]), (null === $key[1] || is_scalar($key[1]) || is_callable([$key[1], '__toString']) ? (string) $key[1] : $key[1]), (null === $key[2] || is_scalar($key[2]) || is_callable([$key[2], '__toString']) ? (string) $key[2] : $key[2])]))))) {
    ...
    {
        $sql = 'SELECT user_id, subscription_id, subscription_type FROM subscription WHERE user_id = :p0 AND subscription_id = :p1 AND subscription_type = :p2';
        try {
        ...
        if ($row = $stmt->fetch(\PDO::FETCH_NUM)) {
            ...
            $obj->hydrate($row);
            SubscriptionTableMap::addInstanceToPool($obj, serialize([(null === $key[0] || is_scalar($key[0]) || is_callable([$key[0], '__toString']) ? (string) $key[0] : $key[0]), (null === $key[1] || is_scalar($key[1]) || is_callable([$key[1], '__toString']) ? (string) $key[1] : $key[1]), (null === $key[2] || is_scalar($key[2]) || is_callable([$key[2], '__toString']) ? (string) $key[2] : $key[2])]));
        }
        $stmt->closeCursor();
    ...
    {
        $this->addUsingAlias(SubscriptionTableMap::COL_USER_ID, $key[0], Criteria::EQUAL);
        $this->addUsingAlias(SubscriptionTableMap::COL_SUBSCRIPTION_ID, $key[1], Criteria::EQUAL);
        $this->addUsingAlias(SubscriptionTableMap::COL_SUBSCRIPTION_TYPE, $key[2], Criteria::EQUAL);
    ...
    public function filterByPrimaryKeys($keys)
    {
        ...
        foreach ($keys as $key) {
            $cton0 = $this->getNewCriterion(SubscriptionTableMap::COL_USER_ID, $key[0], Criteria::EQUAL);
            $cton1 = $this->getNewCriterion(SubscriptionTableMap::COL_SUBSCRIPTION_ID, $key[1], Criteria::EQUAL);
            $cton0->addAnd($cton1);
            $cton2 = $this->getNewCriterion(SubscriptionTableMap::COL_SUBSCRIPTION_TYPE, $key[2], Criteria::EQUAL);
            $cton0->addAnd($cton2);
            $this->addOr($cton0);
        }
     ...
     * Example usage:
     * <code>
     * $query->filterBySubscriptionType('fooValue');   // WHERE subscription_type = 'fooValue'
     * $query->filterBySubscriptionType('%fooValue%', Criteria::LIKE); // WHERE subscription_type LIKE '%fooValue%'
     * </code>
     *
     * @param     string $subscriptionType The value to use as filter.
     * @param     string $comparison Operator to use for the column comparison, defaults to Criteria::EQUAL
     *
     * @return $this|ChildSubscriptionQuery The current query, for fluid interface
     */
    public function filterBySubscriptionType($subscriptionType = null, $comparison = null)
    {
        if (null === $comparison) {
            if (is_array($subscriptionType)) {
                $comparison = Criteria::IN;
            }
        }

        return $this->addUsingAlias(SubscriptionTableMap::COL_SUBSCRIPTION_TYPE, $subscriptionType, $comparison);
    }
    ...
    {
        if ($subscription) {
            $this->addCond('pruneCond0', $this->getAliasedColName(SubscriptionTableMap::COL_USER_ID), $subscription->getUserId(), Criteria::NOT_EQUAL);
            $this->addCond('pruneCond1', $this->getAliasedColName(SubscriptionTableMap::COL_SUBSCRIPTION_ID), $subscription->getSubscriptionId(), Criteria::NOT_EQUAL);
            $this->addCond('pruneCond2', $this->getAliasedColName(SubscriptionTableMap::COL_SUBSCRIPTION_TYPE), $subscription->getSubscriptionType(), Criteria::NOT_EQUAL);
