...
abstract class Order implements ActiveRecordInterface
{
    ...
    /**
     * The value for the collected_at field.
     *
     * @var        DateTime
     */
    ...
    /**
     * Get the [optionally formatted] temporal [collected_at] column value.
     *
     *
     ...
     */
    public function getCollectedAt($format = NULL)
    {
        if ($format === null) {
            return $this->collected_at;
        } else {
            return $this->collected_at instanceof \DateTimeInterface ? $this->collected_at->format($format) : null;
        }
    }
    ...
    public function getDeliveredAt($format = NULL)
    {
        if ($format === null) {
            ...
            return $this->delivered_at instanceof \DateTimeInterface ? $this->delivered_at->format($format) : null;
        }
    }
    ...
    public function setFarmId($v)
    {
        ...
        if ($this->farm_id !== $v) {
            $this->farm_id = $v;
            $this->modifiedColumns[OrderTableMap::COL_FARM_ID] = true;
        }
    ...
    /**
     * Sets the value of [collected_at] column to a normalized version of the date/time value specified.
     *
     * @param  mixed $v string, integer (timestamp), or \DateTimeInterface value.
     *               Empty strings are treated as NULL.
     * @return $this|\Order The current object (for fluent API support)
     */
    public function setCollectedAt($v)
    {
        $dt = PropelDateTime::newInstance($v, null, 'DateTime');
        if ($this->collected_at !== null || $dt !== null) {
            if ($this->collected_at === null || $dt === null || $dt->format("Y-m-d H:i:s.u") !== $this->collected_at->format("Y-m-d H:i:s.u")) {
                $this->collected_at = $dt === null ? null : clone $dt;
                $this->modifiedColumns[OrderTableMap::COL_COLLECTED_AT] = true;
            }
    ...
    public function setDeliveredAt($v)
    {
        ...
        if ($this->delivered_at !== null || $dt !== null) {
            if ($this->delivered_at === null || $dt === null || $dt->format("Y-m-d H:i:s.u") !== $this->delivered_at->format("Y-m-d H:i:s.u")) {
                $this->delivered_at = $dt === null ? null : clone $dt;
                $this->modifiedColumns[OrderTableMap::COL_DELIVERED_AT] = true;
            }
        } // if either are not null
    ...
    public function hydrate($row, $startcol = 0, $rehydrate = false, $indexType = TableMap::TYPE_NUM)
    {
        try {
            ...
            $col = $row[TableMap::TYPE_NUM == $indexType ? 0 + $startcol : OrderTableMap::translateFieldName('UserId', TableMap::TYPE_PHPNAME, $indexType)];
            $this->user_id = (null !== $col) ? (int) $col : null;

            $col = $row[TableMap::TYPE_NUM == $indexType ? 1 + $startcol : OrderTableMap::translateFieldName('FarmId', TableMap::TYPE_PHPNAME, $indexType)];
            ...
            $col = $row[TableMap::TYPE_NUM == $indexType ? 4 + $startcol : OrderTableMap::translateFieldName('CollectedAt', TableMap::TYPE_PHPNAME, $indexType)];
            if ($col === '0000-00-00 00:00:00') {
                $col = null;
            }
            $this->collected_at = (null !== $col) ? PropelDateTime::newInstance($col, null, 'DateTime') : null;

            $col = $row[TableMap::TYPE_NUM == $indexType ? 5 + $startcol : OrderTableMap::translateFieldName('DeliveredAt', TableMap::TYPE_PHPNAME, $indexType)];
            if ($col === '0000-00-00 00:00:00') {
                $col = null;
            }
            $this->delivered_at = (null !== $col) ? PropelDateTime::newInstance($col, null, 'DateTime') : null;

            $col = $row[TableMap::TYPE_NUM == $indexType ? 6 + $startcol : OrderTableMap::translateFieldName('Id', TableMap::TYPE_PHPNAME, $indexType)];
    ...
    protected function doInsert(ConnectionInterface $con)
    {
        ...
        if ($this->isColumnModified(OrderTableMap::COL_FARM_ID)) {
        ...
        if ($this->isColumnModified(OrderTableMap::COL_COLLECTED_AT)) {
            $modifiedColumns[':p' . $index++]  = 'collected_at';
        }
        if ($this->isColumnModified(OrderTableMap::COL_DELIVERED_AT)) {
            ...
            foreach ($modifiedColumns as $identifier => $columnName) {
                switch ($columnName) {
                    ...
                    case 'quantity':
                        ...
                        break;
                    case 'collected_at':
                        $stmt->bindValue($identifier, $this->collected_at ? $this->collected_at->format("Y-m-d H:i:s.u") : null, PDO::PARAM_STR);
                        break;
                    case 'delivered_at':
                        $stmt->bindValue($identifier, $this->delivered_at ? $this->delivered_at->format("Y-m-d H:i:s.u") : null, PDO::PARAM_STR);
                        break;
                    case 'id':
    ...
    public function toArray($keyType = TableMap::TYPE_PHPNAME, $includeLazyLoadColumns = true, $alreadyDumpedObjects = array(), $includeForeignObjects = false)
    {
        ...
        $result = array(
            ...
            $keys[4] => $this->getCollectedAt(),
            $keys[5] => $this->getDeliveredAt(),
            $keys[6] => $this->getId(),
        );
    ...
    public function fromArray($arr, $keyType = TableMap::TYPE_PHPNAME)
    {
        ...
        if (array_key_exists($keys[4], $arr)) {
            $this->setCollectedAt($arr[$keys[4]]);
        }
        if (array_key_exists($keys[5], $arr)) {
            $this->setDeliveredAt($arr[$keys[5]]);
        }
    ...
    public function buildCriteria()
    {
        ...
        if ($this->isColumnModified(OrderTableMap::COL_FARM_ID)) {
            $criteria->add(OrderTableMap::COL_FARM_ID, $this->farm_id);
        }
        if ($this->isColumnModified(OrderTableMap::COL_PRODUCT_ID)) {
        ...
        if ($this->isColumnModified(OrderTableMap::COL_COLLECTED_AT)) {
            $criteria->add(OrderTableMap::COL_COLLECTED_AT, $this->collected_at);
        }
        if ($this->isColumnModified(OrderTableMap::COL_DELIVERED_AT)) {
            $criteria->add(OrderTableMap::COL_DELIVERED_AT, $this->delivered_at);
        }
        if ($this->isColumnModified(OrderTableMap::COL_ID)) {
    ...
    public function copyInto($copyObj, $deepCopy = false, $makeNew = true)
    {
        ...
        $copyObj->setCollectedAt($this->getCollectedAt());
